...
class Demo(nn.Module):
    def __init__(self, args):
        ...
        neutral_path = os.path.join('ckpts/predefined_exp_weights', 'neutral.npy')
        self.neutral_exp = torch.from_numpy(np.load(neutral_path)).cuda()
        self.blink_interval = args.blink_interval
    ...
    def _blink_alpha(self, frame_idx):
        """Returns blend weight toward neutral: 0=base expression, 1=blink (neutral).
        Blink occurs every blink_interval frames, lasts ~5 frames."""
        import math
        if self.blink_interval <= 0:
            return 0.0
        phase = frame_idx % self.blink_interval
        blink_dur = 5  # frames
        if phase < blink_dur:
            return math.sin(phase / blink_dur * math.pi)
        return 0.0
    ...
    def run(self):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(self.lip_vid_target.size(0))):
                ...
                img_target_pose = self.pose_vid_target[:, i % len_pose, :, :, :]

                alpha_exp = self._blink_alpha(i)
                exp_weight = (1.0 - alpha_exp) * self.exp_vid_target + alpha_exp * self.neutral_exp
                img_recon = self.gen.test_EDTalk_A_use_exp_weight(self.img_source, img_target_lip, img_target_pose, exp_weight, h_start)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--blink_interval", type=int, default=100, help='Blink every N frames (0 to disable). Default: 100 (~4s at 25fps)')
