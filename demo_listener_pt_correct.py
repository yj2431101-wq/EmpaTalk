...
def main():
    ...
    parser.add_argument("--listener_tgt", required=True, help="listener video or image path") # for bank_debug
    ...
    args.resume_ckpt = cli.ckpt

    T_MODEL = getattr(args, 'max_frames', 48)
    ...
    ckpt_iter = trainer.resume(args.resume_ckpt)
    print('==> ckpt iteration %d' % (ckpt_iter))
    
    # trainer.gen.eval()
    trainer.gen.train() # for bank debug
    ...
    # ── 6. Prepare T_model inputs (matching training distribution) ──────
    ...
    # ── 8. Interpolate coefficients to original frame count ─────────────
    ...
    # ── 9. Audio2Lip at original frame rate (optional) ──────────────────
    ...
    # ── 10. Listener identity encoding (once) ───────────────────────────
    with torch.no_grad():
        wa_L, _, feats_L, _ = gen.enc(lis_batch, None, None)
    # wa_L: (1, 512), feats_L: list of (1, C_i, H_i, W_i)
    
    # ── save .pt file for debug bank ───────────────────────────
    # print('\n[DEBUG] bank feature')
    # import time
    # save_path = f"/home/yjcho/EmpaTalk/bank_debug/inference-code-dia/bank_{int(time.time() * 1000)}.pt"
    # torch.save({
    ...
    # }, save_path)
    # print("[DEBUG] bank feature saved at: ", save_path)
