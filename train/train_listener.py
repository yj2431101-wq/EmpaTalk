...
def save_loss_plot(history: dict, path: str):
    keys = ["vgg", "l1", "adv", "kl", "mel", "bank", "dis"]
...
def write_loss(step, vgg, l1, adv, kl, mel, bank, d, writer: SummaryWriter):
...
def main(args):
    ...
    for epoch in epoch_bar:
        ...
        for batch in batch_bar:
            ...
            # Generator step
            vgg_loss, l1_loss, adv_loss, kl_loss, mel_loss, bank_loss, img_recon = trainer.gen_update(
            ...
            if is_main:
                ...
                loss_history["mel"].append(mel_loss.item())
            ...
            if is_main and current_iter % args.log_iter == 0:
                write_loss(current_iter, vgg_loss, l1_loss, adv_loss, kl_loss, mel_loss, bank_loss, d_loss, writer)
            ...
            if is_main and current_iter % args.display_freq == 0:
                tqdm.write(
                    ...
                    f"mel={mel_loss.item():.4f}  bank={bank_loss.item():.4f}  "
                    f"d={d_loss.item():.4f}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_bank", type=float, default=5.0,
                        help="Bank loss weight: L1 between bank-predicted and GT listener features")
