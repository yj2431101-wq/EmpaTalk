...
class TrainerListener(nn.Module):
    ...
    def gen_update(
        ...
        Returns:
            vgg_loss, l1_loss, adv_loss, kl_loss, mel_loss, bank_loss, img_recon
        """
        self.gen.train()
        self.gen.zero_grad()
    ...
    def resume(self, ckpt_path: str) -> int:
        ...
        motion_missing = [k for k in missing if k in ("pose_motion.weight", "pose_motion.bias",
                                                       "exp_motion.weight",  "exp_motion.bias")]
        other_missing  = [k for k in missing if k not in audio_missing + motion_missing]
        if other_missing:
        ...
        if motion_missing:
            print(f"  pose_motion/exp_motion not in ckpt (randomly init'd): {len(motion_missing)} keys")
        if unexpected:
        ...
        new_params_added = bool(audio_missing or motion_missing)
        if not new_params_added:
